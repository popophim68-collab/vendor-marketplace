...
class VendorServiceProvider extends ServiceProvider
{
    ...
    private function registerShortcodes(): void
    {
        ...
        // ── شورت كود تسجيل البائع (مُحدّث لدعم التسجيل متعدد الخطوات) ──
        add_shortcode('vmp_vendor_register', function (): string {
            ...
            if ($current_user_id) {
                ...
                // إذا كان المستخدم ليس بائعاً لكنه مسجل بالفعل (زبون/مشترك)
            ...
            // ── بيانات النموذج من الجلسة (للاستئناف) ──
            $session_key = 'vmp_vendor_register_data';
            $form_data = isset($_SESSION[$session_key]) && is_array($_SESSION[$session_key]) ? $_SESSION[$session_key] : [];
            $current_step = isset($_SESSION['vmp_vendor_register_step']) ? (int) $_SESSION['vmp_vendor_register_step'] : 1;
            $errors = isset($_SESSION['vmp_vendor_register_errors']) && is_array($_SESSION['vmp_vendor_register_errors']) ? $_SESSION['vmp_vendor_register_errors'] : [];

            // ── مسح الأخطاء بعد عرضها ──
            unset($_SESSION['vmp_vendor_register_errors']);

            // ── المتغيرات المشتركة للقوالب ──
            $template_vars = compact('step_key', 'form_data', 'errors', 'current_user', 'settings', 'plans', 'default_plan_id', 'terms_url', 'current_step');

            // دمج المتغيرات المشتركة مع متغيرات كل خطوة
    ...
    private function registerAssets(): void
    {
        add_action('wp_enqueue_scripts', function (): void {
            ...
            if ($current_page === 'dashboard' && !empty($GLOBALS['post'])) {
                $content = $GLOBALS['post']->post_content ?? '';
                error_log('VMP DEBUG: post_content length=' . strlen($content) . ', post_ID=' . ($GLOBALS['post']->ID ?? 'none'));
                if (has_shortcode($content, 'vmp_vendor_register')) {
                    error_log('VMP DEBUG: Found vmp_vendor_register shortcode, setting current_page=register');
            ...
            if ($current_page === 'dashboard') {
                ...
                if ($register_page_id) {
                    ...
                    if ($current_page_id == $register_page_id) {
                        error_log('VMP DEBUG: Current page ID (' . $current_page_id . ') matches register page ID (' . $register_page_id . '), setting current_page=register');
                        $current_page = 'register';
                    }
                }
            }
            ...
            if ($current_page === 'dashboard') {
                ...
                if (strpos($request_uri, 'vendor-register') !== false || strpos($request_uri, 'تسجيل-بائع') !== false) {
                    error_log('VMP DEBUG: URL contains vendor-register, setting current_page=register');
                    $current_page = 'register';
                }
            }
